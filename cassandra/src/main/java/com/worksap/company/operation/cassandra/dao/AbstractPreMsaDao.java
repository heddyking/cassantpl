...
import com.worksap.company.operation.cassandra.dto.PreMsaDto;
 ...
 * @generated 
 */
public abstract class AbstractPreMsaDao {
    ...
    protected final Mapper<PreMsaDto> mapper;

    public AbstractPreMsaDao(Session session, Future<Session> sessionAsync, MappingManager manager) {
        this.session = session;
        this.sessionAsync = sessionAsync;
        this.manager = manager;
        this.mapper = manager.mapper(PreMsaDto.class);
    }
     ...
     * @param dto
     */
    public void save(PreMsaDto dto) {
        mapper.save(dto);
    }
     ...
     * @param dto
     */
    public Future<Void> saveAsync(PreMsaDto dto) {
     ...
     * @param {primarykeys}
     */
    public PreMsaDto get(String tenantId, String landscape, UUID ts) {
     ...
     * @param {primarykeys}
     */
    public Future<PreMsaDto> getAsync(String tenantId, String landscape, UUID ts) {
     ...
     * @param dto
     */
    public void delete(PreMsaDto dto) {
        mapper.delete(dto);
    }
     ...
     * @param dto
     */
    public Future<Void> deleteAsync(PreMsaDto dto) {
     ...
     * @param list
     */
    public void saveBatch(List<PreMsaDto> list) {
     ...
     * @param list
     */
    public Future<Void> saveBatchAsync(List<PreMsaDto> list) {
        BatchStatement batch = new BatchStatement();
        list.stream().forEach(dto -> batch.add(mapper.saveQuery(dto)));
     ...
     * @return
     */
    public boolean saveLwt(PreMsaDto dto){
        Statement query = QueryBuilder.insertInto(PreMsaTable._NAME)
                .value(PreMsaTable.TENANT_ID, dto.getTenantId())
                .value(PreMsaTable.LANDSCAPE, dto.getLandscape())
                .value(PreMsaTable.TS, dto.getTs())
                .ifNotExists()
                ;
     ...
     * @return
     */
    public Future<Boolean> saveLwtAsync(PreMsaDto dto){
        Statement query = QueryBuilder.insertInto(PreMsaTable._NAME)
                .value(PreMsaTable.TENANT_ID, dto.getTenantId())
                .value(PreMsaTable.LANDSCAPE, dto.getLandscape())
                .value(PreMsaTable.TS, dto.getTs())
     ...
     */
    public List<PreMsaDto> getAll() {
        Statement query = QueryBuilder
                .select()
                .all()
                .from(PreMsaTable._NAME);
     ...
     */
    public Future<List<PreMsaDto>> getAllAsync() {
        Statement query = QueryBuilder
                .select()
                .all()
                .from(PreMsaTable._NAME);
     ...
     */
    public List<PreMsaDto> getByPk(/*partition key*/ String tenantId) {
	    Statement query = QueryBuilder
                .select()
                .all()
                .from(PreMsaTable._NAME)
				.where(QueryBuilder.eq(PreMsaTable.TENANT_ID,tenantId))
				;
		return mapper.map(session.execute(query)).all();
	}
     ...
     * @param {partition keys}
     */
    public Future<List<PreMsaDto>> getByPkAsync(/*partition key*/ String tenantId) {
	    Statement query = QueryBuilder
                .select()
                .all()
                .from(PreMsaTable._NAME)
     ...
     * @param {partition keys}
     */
    public List<PreMsaDto> getByPkCk1(/*partition key*/ String tenantId
	    ...
	    Statement query = QueryBuilder
                ...
                .from(PreMsaTable._NAME)
				.where(QueryBuilder.eq(PreMsaTable.TENANT_ID,tenantId))
				.and(QueryBuilder.eq(PreMsaTable.LANDSCAPE,landscape))
				;
		return mapper.map(session.execute(query)).all();
	}
     ...
     * @param {partition keys}
     */
    public Future<List<PreMsaDto>> getByPkCk1Async(/*partition key*/ String tenantId
	                            /*clustering key*/ , String landscape) {
	    Statement query = QueryBuilder
                .select()
                .all()
     ...
     * @param {partition keys}
     */
    public List<PreMsaDto> getByCk1(/*clustering key*/ String landscape) {
	    Statement query = QueryBuilder
                .select()
                .all()
                .from(PreMsaTable._NAME)
                .allowFiltering()
				.where(QueryBuilder.eq(PreMsaTable.LANDSCAPE,landscape))
				;
		return mapper.map(session.execute(query)).all();
	}
     ...
     * @param {partition keys}
     */
    public Future<List<PreMsaDto>> getByCk1Async(/*clustering key*/ String landscape) {
	    Statement query = QueryBuilder
                .select()
                .all()
                .from(PreMsaTable._NAME)
     ...
     * @param {partition keys}
     */
    public List<PreMsaDto> getByPkCk2(/*partition key*/ String tenantId
	    ...
	    Statement query = QueryBuilder
				...
				.where(QueryBuilder.eq(PreMsaTable.TENANT_ID,tenantId))
				.and(QueryBuilder.eq(PreMsaTable.LANDSCAPE,landscape))
				.and(QueryBuilder.eq(PreMsaTable.TS,ts))
				;
		return mapper.map(session.execute(query)).all();
	}
     ...
     * @param {partition keys}
     */
    public Future<List<PreMsaDto>> getByPkCk2Async(/*partition key*/ String tenantId
	                            /*clustering key*/ , String landscape, UUID ts) {
	    Statement query = QueryBuilder
                .select()
                .all()
     ...
     * @param {partition keys}
     */
    public List<PreMsaDto> getByCk2(/*clustering key*/ String landscape, UUID ts) {
	    Statement query = QueryBuilder
                .select()
                .all()
                .from(PreMsaTable._NAME)
     ...
     * @param {partition keys}
     */
    public Future<List<PreMsaDto>> getByCk2Async(/*clustering key*/ String landscape, UUID ts) {
	    Statement query = QueryBuilder
                .select()
                .all()
                .from(PreMsaTable._NAME)
     ...
     * @param pagingState
     */
    public Pair<PagingState,List<PreMsaDto>> getNextPage(Integer size, Optional<String> pagingState) {
        Statement query = QueryBuilder
                .select()
                .all()
                .from(PreMsaTable._NAME);
     ...
     * @param query
     */
    protected Pair<PagingState,List<PreMsaDto>> queryNextPage(
        ...
        Result<PreMsaDto> results = mapper.map(rs);
        PagingState newPagingState = results.getExecutionInfo().getPagingState();
        List<PreMsaDto> resultList = new ArrayList<PreMsaDto>();
        for (PreMsaDto dto : results) {
            resultList.add(dto);
            if (results.getAvailableWithoutFetching() == 0) {
                break;
            }
        }
        return new Pair<PagingState, List<PreMsaDto>>(newPagingState,resultList);
    }
     ...
     * @param pagingState
     */
    public Future<Pair<PagingState,List<PreMsaDto>>> getNextPageAsync(
     ...
     * @param query
     */
    protected Future<Pair<PagingState,List<PreMsaDto>>> queryNextPageAsync(
            Integer size, Optional<String> pagingState, Statement query) {
        query.setFetchSize(size);
        if (pagingState.isPresent()) {
            query.setPagingState(PagingState.fromString(pagingState.get()));
     ...
     * @param page
     */
    public Future<Pair<PagingState,List<PreMsaDto>>> getCertainPageAsync(Integer size, Integer page) {
        Statement query = QueryBuilder
                .select()
                .all()
                .from(PreMsaTable._NAME);
     ...
     * @param query
     */
    protected Future<Pair<PagingState,List<PreMsaDto>>> queryCertainPageAsync(
     ...
     */
    protected Future<List<PreMsaDto>> transferFutureResultSet(
            Future<ResultSet> futureResultSet) {
        Future<List<PreMsaDto>> futureResutlList = Futures.transform((ListenableFuture<ResultSet>)futureResultSet,
                (Function<ResultSet, List<PreMsaDto>>) rs -> {
                    Result<PreMsaDto> results = mapper.map(rs);
                    PagingState newPagingState = results.getExecutionInfo().getPagingState();
                    List<PreMsaDto> resultList = new ArrayList<PreMsaDto>();
                    for (PreMsaDto dto : results) {
     ...
     * @param futureResultSet
     */
    protected Future<Pair<PagingState,List<PreMsaDto>>> transferFutureResultSetWithState(
            Future<ResultSet> futureResultSet) {
        Future<Pair<PagingState,List<PreMsaDto>>> futureResutlList = Futures.transform((ListenableFuture<ResultSet>)futureResultSet,
                (Function<ResultSet, Pair<PagingState,List<PreMsaDto>>>) rs -> {
                    Result<PreMsaDto> results = mapper.map(rs);
                    PagingState newPagingState = results.getExecutionInfo().getPagingState();
                    List<PreMsaDto> resultList = new ArrayList<PreMsaDto>();
                    for (PreMsaDto dto : results) {
